...
from src.models.components.per_patch_with_backbone import PerPatch
...
class MVT(LightningModule):
    ...
    def forward(self, input: torch.Tensor):
        ...
        x = torch.empty((self.hparams.batch_size * self.hparams.num_views * self.hparams.num_patches,
                         self.hparams.hidden_dimension))
        ...
        # Unflatten x by one dimension.
        x = torch.reshape(x, (
        self.hparams.batch_size, self.hparams.num_views, self.hparams.num_patches, self.hparams.hidden_dimension))
        ...
        x = torch.reshape(x, (
        self.hparams.batch_size, self.hparams.num_views, self.hparams.num_patches + 1, self.hparams.hidden_dimension))
    ...
    def infer(self, input: Any):
        input_ = input
        for i in range(self.hparams.batch_size-1):
            input_ = torch.cat((input_, input))
        preds = self.predict(input_)
        return preds[0]
    ...
    def configure_optimizers(self):
        ...
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optim, mode="max", patience=5)
